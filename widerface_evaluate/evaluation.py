...
def evaluation_mAP(pred, gt_path, iou_thresh_list=np.arange(0.5,0.95,0.05)):
    pred = get_preds(pred)
    norm_score(pred)
    facebox_list, event_list, file_list, hard_gt_list, medium_gt_list, easy_gt_list = get_gt_boxes(gt_path)
    event_num = len(event_list)
...
if __name__ == '__main__':
    ...
    for target_size in arr_target_size:
        map = evaluation_mAP(f"{args.pred}{target_size}", args.gt)
        arr_map.append(map)
        print(f"map{target_size}: {map}")
    print(f"map final:{np.mean(arr_map)}")
