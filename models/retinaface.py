...
from models.net import IntermediateLayerGetter
...
class ClassHead(nn.Layer):
    ...
    def forward(self,x):
        out = self.conv1x1(x)
        out = paddle.transpose(out,[0,2,3,1])#out.transpose(0,2,3,1)#.contiguous()
...
class BboxHead(nn.Layer):
    ...
    def forward(self,x):
        out = self.conv1x1(x)
        out = paddle.transpose(out,[0,2,3,1])#.contiguous()
...
class LandmarkHead(nn.Layer):
    ...
    def forward(self,x):
        out = self.conv1x1(x)
        out = out = paddle.transpose(out,[0,2,3,1])#out.transpose(0,2,3,1)#.contiguous()
...
class RetinaFace(nn.Layer):
    def __init__(self, cfg = None, phase = 'train'):
        ...
        if cfg['name'] == 'mobilenet0.25':
            ...
            if cfg['pretrain']:
                checkpoint = paddle.load("./weights/mobilenetV1X0.25_pretrain.tar")
                #from collections import OrderedDict
        ...
        self.body = IntermediateLayerGetter(backbone, cfg['return_layers']) #_utils.IntermediateLayerGetter(backbone, cfg['return_layers'])
